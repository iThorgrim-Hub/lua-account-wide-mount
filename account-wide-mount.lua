...
CharDBExecute('CREATE TABLE IF NOT EXISTS `'..accountMount.Config.dbName..'`.`account_mount_link` ( `accountid` INT(10) NOT NULL, `spellid` INT(10), PRIMARY KEY (`accountid`, `spellid`));');
...
function accountMount.getMount(event, player)
	...
	if (pLevel >= 20) then
		if (not (accountMount[pAccId])) then
			...
			local getAccountMount = CharDBQuery('SELECT spellid FROM `'..accountMount.Config.dbName..'`.`account_mount_link` WHERE accountid = '..pAccId..';');
	...
	if (pLevel >= 20) then
		if (accountMount.newSpells[pAccId]) then
			for k, v in pairs (accountMount.newSpells[pGuid]) do
				local setAccountMount = CharDBQuery('INSERT INTO `'..accountMount.Config.dbName..'`.`account_mount_link` VALUES ('..pAccId..', '..v..')');
